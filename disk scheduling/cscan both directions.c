...
void cscanDiskScheduling(int requests[], int n, int head, int disk_size, int d) {
    int totalHeadMovement = 0;
    int sortedRequests[n + 2];  // Include boundaries (0 and disk_size - 1)
    int i, j, pos = 0;
    ...
    sortedRequests[n] = 0;                    // Start boundary
    sortedRequests[n + 1] = disk_size - 1;     // End boundary
    n += 2;
    ...
    if (d == 1) {  // Move Right
        ...
        for (i = n-1; i >= pos; i--) {        // Then move right
            printf(" -> %d", sortedRequests[i]);
            totalHeadMovement += abs(sortedRequests[i] - head);
            head = sortedRequests[i];
        }
...
void main() {
    ...
    cscanDiskScheduling(requests, n, head, disk_size, d);
}
